...
const MainProfile = ({ user }) => {
  ...
  useEffect(() => {
    fetch(`https://twitter-clone-xnsk.vercel.app/userpost?email=${user?.email}`)
      .then(res => res.json())
      .then(data => {
        setPosts(data);
      })
  ...
  const handleUploadCoverImage = e => {
    ...
    axios.post("https://api.imgbb.com/1/upload?key=6d2c7a56dd27184ac6f798b31a2db92f", formData)
      .then(res => {
        ...
        if (url) {
          fetch(`https://twitter-clone-xnsk.vercel.app/userUpdates/${user?.email}`, {
            method: "PATCH",
            headers: {
